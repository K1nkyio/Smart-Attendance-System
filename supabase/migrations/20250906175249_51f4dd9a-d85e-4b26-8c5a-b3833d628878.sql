...
FOR INSERT 
WITH CHECK (
  public.get_current_user_type() = 'instructor' 
...
ON public.classes 
FOR UPDATE 
USING (instructor_id = public.get_current_user_profile_id());

-- Create RLS policies for class enrollments
CREATE POLICY "Students can view their own enrollments" 
ON public.class_enrollments 
FOR SELECT 
USING (student_id = public.get_current_user_profile_id());
...
USING (
  EXISTS (
    ...
    AND classes.instructor_id = public.get_current_user_profile_id()
  )
);
...
FOR INSERT 
WITH CHECK (
  public.get_current_user_type() = 'student'
  AND student_id = public.get_current_user_profile_id()
);
...
ON public.attendance_records 
FOR INSERT 
WITH CHECK (student_id = public.get_current_user_profile_id());
